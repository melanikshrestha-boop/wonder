...
function ReviewEditor({ job, stage, draft, setDraft, regenPrompt, setRegenPrompt, busy, canRegenerate, onAction }) {
  ...
  return (
    <div className="import-editor">
      ...
      <div className="import-fields">
        ...
        {isCrop ? <p className="import-card__detail">Check that this crop contains the complete intended item. Approving it continues to the wardrobe details.</p> : isGarment ? (
          <>
        ...
        {!isCrop && canRegenerate && <div className="import-field import-regenerate-field">
        ...
        <div className="import-actions">
          ...
          {!isCrop && canRegenerate && <button className="import-button" disabled={busy} onClick={() => onAction("regenerate", regenPrompt)}><ArrowCounterClockwise size={14} /> Regenerate</button>}
...
export function WardrobeImportFlow({ onGarmentApproved, onModeledApproved }) {
  ...
  const perform = async (job, stage, action, prompt = "") => {
    ...
    try {
      if (stage === "garment" && action === "approve") {
        ...
        const importedId = `import-${job.id}`;
        const importedItems = await api("/api/import/wardrobe");
        const importedItem = importedItems.find((item) => item.id === importedId);
        const garmentPath = `/api/import/library/${importedId}-garment.png`;
        onGarmentApproved?.(importedItem || { id: importedId, ...metadata, image: garmentPath, thumbnail: garmentPath, modeledImage: null, palette: [metadata.color, metadata.secondaryColor].filter(Boolean), importJobId: job.id });
  ...
  const activeStatus = setupRequired ? { tone: "error", text: "Import unavailable" } : active ? deriveStatus(active) : notice;
