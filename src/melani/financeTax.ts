...
function isSelfEmploymentLike(tx: FinanceTx): boolean {
  return /\b(1099[- ]?nec|schedule c|self[- ]employ(?:ed|ment)?|freelanc(?:e|er|ing)?|contractor|consult(?:ant|ing|ancy)?|client payment|invoice|stripe|youtube|sponsor(?:ship|ed)?|ad revenue|substack|creator revenue|business income)\b/.test(
    transactionText(tx)
  );
}
